...
export function TrendsView({ foods, logs, dailyGoals }: TrendsViewProps) {
  ...
  const todayNutrients = useMemo(() => getNutrientsForDate(today), [logs, foods, today]);
  ...
  // Pie chart data - macro calorie distribution
  const pieChartData = useMemo(() => {
    // Get total grams of each macro
    const carbs = (todayNutrients['carbohydrates'] || 0) + (todayNutrients['sugars'] || 0);
    const protein = todayNutrients['proteins'] || 0;
    const fat = (todayNutrients['fat'] || 0) + (todayNutrients['saturated-fat'] || 0) + (todayNutrients['unsaturated-fat'] || 0);
